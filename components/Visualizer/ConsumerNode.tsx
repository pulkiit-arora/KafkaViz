...
export const ConsumerNode: React.FC<ConsumerNodeProps> = ({ 
  ...
  return (
    <div className="bg-white border-l-4 border-orange-500 shadow-md rounded-r-lg p-4 w-64 mb-4 relative group">
      <button 
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <div>
          <h4 className="font-bold text-slate-800">{consumer.name}</h4>
          <p className="text-xs text-slate-500">Group: {consumer.groupId}</p>
        </div>
      </div>

      <div className="mb-3 space-y-1">
        <label className="text-[10px] uppercase font-bold text-slate-500">Subscribed Topic</label>
        <select 
          className="w-full text-sm border-slate-200 rounded-md p-1.5 bg-slate-50 text-slate-700 focus:ring-2 focus:ring-orange-500 focus:outline-none"
        ...
        <AnimatePresence mode='wait'>
          {consumer.lastConsumedMessage ? (
            <motion.div 
              ...
              exit={{ opacity: 0, y: 10 }}
              className="absolute inset-0 bg-green-50 border border-green-200 rounded flex items-center justify-between px-3"
            >
              ...
              <span className="text-[9px] text-green-500 font-bold uppercase tracking-wider">Processed</span>
            </motion.div>
          ) : (
            <div className="absolute inset-0 flex items-center justify-center text-xs text-slate-400 italic bg-slate-50 rounded border border-dashed border-slate-200">
              Waiting for data...
            </div>
          )}
        </AnimatePresence>
      ...
      <div className="flex gap-2">
        ...
        >
          {isAutoConsuming ? <Pause size={14} /> : <Play size={14} />}
